...
namespace PasswordManagementSystem.Web.Controllers
{
    public class AdminController : Controller
    {
        ...
        public AdminController(IUserService userService, IRoleService roleService, ILogService logService, IPasswordService passwordService, ILabelService labelService)
        {
        ...
        public async Task<IActionResult> CompanyLogs(int page = 1, int pageSize = 10, bool isAjax = false)
        {
            bool loginStatus = await controlForLoginStatus();
            if (!loginStatus)
            {
            ...
            var companyLogs = await _logService.GetLogsByCompanyId(currentUser.CompanyId, page, pageSize);
            var totalItems = await _logService.GetTotalLogCount(currentUser.CompanyId);
            ...
            var model = new CompanyLogsViewModel
            {
                Logs = companyLogs.ToList(),
                CurrentPage = page,
                TotalPages = totalPages
            };
        ...
        public async Task<IActionResult> PasswordListing(int page = 1, int pageSize = 10, bool isAjax = false)
        {
            var sessionUserId = HttpContext.Session.GetInt32("UserId");
            ...
            var passwords = await _passwordService.GetCompanyPasswords(companyId, page, pageSize);

            var totalItems = await _passwordService.GetTotalPasswordCount(companyId);
            var totalPages = (int)Math.Ceiling((double)totalItems / pageSize);
        ...
        public async Task<IActionResult> AddUpdatePassword()
        {
            ...
            var passwords = await _passwordService.GetCompanyPasswords(companyId,1,10);
        ...
        public async Task<IActionResult> CheckUserPassword(string userPassword)
        {
            ...
            bool control = await _userService.CheckUserPassword(sessionUserId.Value, userPassword);
            if (control)
            {
